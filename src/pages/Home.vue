<template>
  <div class="page-index" :style="{ position: 'relative', height: tableHeight + 'px' }">
    <el-divider content-position="right">
      欢迎回来:
      <font color="#FF4500">{{name == null ? "" : name}}</font>
    </el-divider>
    <!-- partial:index.partial.html -->
    <el-row type="flex" justify="center" align="middle">
      <div class="card" @click="cardClick">
        <div class="card__illustration" v-on:mousemove="mousemove" v-on:mouseout="mouseout">
          <svg viewBox="0 0 100 100">
            <g class="face-group">
              <path class="face" d="m38.977 59.074c0 2.75-4.125 2.75-4.125 0s4.125-2.75 4.125 0" />
              <path class="face" d="m60.477 59.074c0 2.75-4.125 2.75-4.125 0s4.125-2.75 4.125 0" />
              <path
                class="face"
        ...
        <div class="card__content" v-on:animationend="animationend">
          <div class="card__info">
            <h2 class="card__name">{{name == null ? "" : name}}</h2>
            <!-- <h3 class="card__title">Ant Collector</h3> -->
            <p class="card__desc">欢迎使用微拍堂测试平台!</p>
          </div>
          <div class="card__actions">
            <!-- <button class="btn btn--action">like</button> -->
            <button class="btn btn--action" @click.stop="closeCard">close</button>
          </div>
        </div>
        <button class="btn btn--close" @click.stop="closeCard">
  ...
  methods: {
    mouseout() {
      const illustration = document
        .querySelector(".card")
        .querySelector(".card__illustration");
      illustration.style.setProperty("--x", 0);
      illustration.style.setProperty("--y", 0);
    },
    mousemove(e) {
      const illustration = document
        .querySelector(".card")
        .querySelector(".card__illustration");
      const r = illustration.getBoundingClientRect();
    ...
    animationend() {
      document.querySelector(".card").classList.remove(this.cls.closing);
    },
    closeCard() {
      document.querySelector(".card").classList.add(this.cls.closing);
      document.querySelector(".card").classList.remove(this.cls.active);
    },
    cardClick() {
      document.querySelector(".card").classList.add(this.cls.active);
    },
    initData() {
      this.adminData = JSON.parse(localStorage.getItem("localUsr"));
    },
    // 动态高度
    hh() {
...
.is-active .card__illustration,
.is-closing .card__illustration {
  pointer-events: none;
}
.is-active .card__illustration svg,
.is-active .card__illustration .face-group,
.is-closing .card__illustration svg,
