...
<template>
  <div class="page page-reimComplete">
    <div class="page-search-bar">
      ...
      <el-button type="primary" size="small" @click="completeRemit">打款</el-button>
      <span style="margin-left:20px;font-weight:bold">已报销总金额: {{ totalAmount }}</span>
      <span style="margin-left:20px;font-weight:bold">已打款总金额: {{ remitedMoney }}</span>
      <span style="margin-left:20px;font-weight:bold">剩余总金额: {{ remainMoney }}</span>
    </div>
...
export default {
  ...
  methods: {
    // 打款
    completeRemit () {
      this.$msgbox.confirm('是否确认打款？本次打款只做记录？？', '提示', {
        type: 'warning',
        center: true
      }).then(() => {
        this.$service.completeRemit({ incrMoney: this.incrMoney }).then(() => {
    ...
    // 导出Excel
    async getExcelDataComplete () {
      const res = await this.$service.getExcelDataComplete()
      return res.data
    },
    // 获取表格数据
    getTableData () {
      ...
      this.tableData = []
      this.$service.getAllReimComplete(this.searchData).then(res => {
        this.tableData = res.data
        this.total = res.totalCount
        this.totalAmount = res.extra.totalAmount
        this.remitedMoney = res.extra.remitedMoney
        this.remainMoney = res.extra.remainMoney
      }).finally(() => {
